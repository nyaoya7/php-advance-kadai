...
// MAMPを利用しているMacユーザーの方は、''ではなく'root'を代入してください
 ...
 try {
    ...
    if ($order === 'desc') {
        $sql_select = 'SELECT * FROM products WHERE book_name LIKE :keyword ORDER BY updated_at DESC';
    } else {
        $sql_select = 'SELECT * FROM products WHERE book_name LIKE :keyword ORDER BY updated_at ASC';
    }
